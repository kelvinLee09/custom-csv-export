...
if ( !class_exists( 'WC_Order_Export_Admin' ) ) {
	class WC_Order_Export_Engine {
		...
		protected static function init_formater( $mode, $settings, $fname, &$labels, &$static_vals, $offset ) {
			...
			include_once dirname( __FILE__ ) . "/formats/storage/interface-woe-formatter-storage.php";
			include_once dirname( __FILE__ ) . "/formats/storage/class-woe-formatter-storage-row.php";
			include_once dirname( __FILE__ ) . "/formats/storage/class-woe-formatter-storage-column.php";
			include_once dirname( __FILE__ ) . "/formats/storage/class-woe-formatter-storage-csv.php";
			include_once dirname( __FILE__ ) . "/formats/storage/class-woe-formatter-storage-summary-session.php";
	...
	//			self::maybe_output_summary_report( $formater );
		...
		public static function build_file_full( $settings, $filename = '', $limit = 0, $order_ids = array() ) {
			global $wpdb, $default_main_settings;
